...
namespace TextTemplate
{
    class TextTemplateVisitor : TextTemplateParserBaseVisitor<object>
    {
        ...
        public override object VisitCompilationUnit([NotNull] TextTemplateParser.CompilationUnitContext ctx)
        {
            ...
            ArrayList<object> result = (ArrayList<object>)VisitChildren(ctx);
            return null;
        }
        ...
        doCompose = function(parts, output, indent){
            ...
            for (let iParts = 0; iParts < parts.length; iParts++){
                ...
                if (item == null){
                    ...
                    let bulletInTheOutput = lines[lines.length - 1].replace(/^([ \t]*(\x01\{\.\})?).<<<asterisk>>>/s,'$1'); 
                    ...
                    if (item.type == 'bullet'){
                        ...
                        if (item.list.length > 0){
                            ...
                            if (typeof nextLine == "string"){
                                bNextLineStartsWithBullet = /^\s*\x01{.}.<<<asterisk>>>/s.test(nextLine);
                            }
                        }
                        if (!!indent && indent.includes('\x01{.}')){
                            ...
                            if (item.list.length > 0){
                                ...
                                nextIndent = typeof nextLine == 'string'  ?  nextLine.replace(/^([ \t]*).<<<asterisk>>>/s, '$1') : '';
                            }
